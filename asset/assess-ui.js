/* ============================================================================
   Pipe Assessor — shared assessment workbench view (classic script, no build).
   The full B31.3 assessment presentation extracted from the old calculator
   page: Integrity Status banner, wall-thickness cross-section SVG (with drag
   handle + hover highlighting), compliance verdicts, ERF gauge, results grid,
   PCC-2 Repair Advisor, and the substituted References & Equations blocks —
   all driven by a computeB313() result (asset/shared.js).

   Load AFTER shared.js with a plain non-defer <script> tag (same rationale:
   page scripts reference these globals at parse time). Depends only on
   shared.js globals (paFmt) — no Basecoat/Leaflet/Supabase access.

   API:
     paCreateAssessView(root, opts) → view
       root  container element; the workbench markup is injected into it
       opts.sections     array of section keys to render, in order
                         (default: status, scope, svg, results, advisor, equations)
       opts.collapsed    section keys rendered as closed <details> (default [])
       opts.onDepthDrag  fn(depth_mm) — enables the SVG drag handle; the host
                         updates its own inputs and re-renders (view keeps no
                         input state of its own)
       view.render(res, ctx)  res = computeB313() result or null;
                              ctx = { neutral: bool  (cold form, show "—" not ERROR),
                                      nps: string    (for the API 574 lookup line) }
       view.getSvg()          the live cross-section <svg> (for PDF rasterization)

     paAdvisorItems(res)  pure PCC-2 advisor content (shared with PDF builders)
     PA_SCOPE_HTML / PA_SCOPE_TEXT  single source of the Scope & Limitations text

   Multiple instances per document are supported: the markup carries no element
   ids — every lookup is data-k scoped to the instance root. Component CSS
   lives in asset/theme.css ("Assessment workbench" section).
   ============================================================================ */
...
const PA_SCOPE_TEXT = PA_SCOPE_HTML.replace(/<[^>]+>/g, '').replace(/&amp;/g, '&').replace(/&mdash;/g, '—');
...
function paSvgToPng(svgEl, scale) {
  scale = scale || 3;
...
async function paCrossSectionPng(res, scale) {
...
function paCreateAssessView(root, opts) {
  opts = opts || {};
